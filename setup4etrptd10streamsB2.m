...
dz2 = @(n) 150 + 150/200*(0:(n-1))' +1.2*exp((((0:(n-1))-150)/22).^.9)';
z2 = 1e5+cumsum(real(dz2(410)))-real(dz2(1));
h_atm = z2(:);
%  $$$ % numel(z_atm)/numel(h_atm);
% $$$ dz4 = @(n) 150 + 2.2e3 + 2.2e3*tanh(((0:(n-1))-350)/50);
% $$$ dz4 = @(n) 150 + 1.e3*(1+tanh(((0:(n-1))-350)/105)); 
% $$$ z4 = cumsum(dz4(475))-dz(1)+1e5;
% $$$ h_atm = z4(:); %z_atm(:); % make sure it's a column vector
%dz4 = @(n) 150 + 1.e3*(1+tanh(((0:(n-1))-350)/105)); 
%z4 = cumsum(dz4(515))-dz4(1)+1e5;
%h_atm = z4(:); %z_atm(:); % make sure it's a column vector
...
[lon,lat,alt,xB,yB,zB] = plotB_latlong_conjugates(latitude,...
                                                  ...
                                                  t4B);
view(156,60.3661)
axis([-8869 37015 -15853 30031 -14069 16520])

[Bt, Bp, Br] = igrf(datenum(t4B(1:3)),lon,lat,alt);
B0 = (Bt.^2+Bp.^2+Br.^2).^(1/2)*1e-9;
B = interp1(alt(1:800),B0(1:800),h_atm/1e3,'pchip');
if plot_figs
  figure
  plot(B,h_atm/1e3,'linewidth',2)
  xlabel('B (T)','fontsize',14)
