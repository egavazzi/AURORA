function [All_I_lambda,It_Mm] = combine_n_extract_I_dt_Mm5(root_dir,RunDirs)
...
ax_ff_IMA     = [3  12 0 0.9]; % [3 12 0 1.05];
ax_Ef_IMA     = [2   9 0 0.9]; % [2  9 0 1.05];
ax_dtheta_IMA = [0  70 0 0.9]; % [0 16 0 1.05];
ax_ExcT_IMA   = [0  20 0 0.9]; 
ax_ExcT_dt    = [0  20 0 30]; 
ax_z0_IMA     = [1500 4500 0 0.9];
...
ph_f = plot_pars_n_pars(pars_of_ItMm,...  % Etrp-parameters array
                        I_mamp,...        % Y-values, relative modulation amplitude
...
phO = plot(E_levelsOnN2(3:end,2),...
           I_mamp(i_Ef3_z3000,[3 4 5 6]),...
           '-','linewidth',2);
hold on
phN2 = plot(E_levelsOnN2(1:2,2),...
            I_mamp(i_Ef3_z3000,[1 2]),...
            '--','linewidth',2);
for i1 = 1:6,
  phP(i1) = plot(-1,0.1*i1,'.','markersize',31,'color',cmp(i1,:));
end

ph7774O = plot(E_levelsOnN2(3,2),I_mamp(i_Ef3_z3000,7),'s','color',cmp(2,:),'linewidth',2);
ph7774O2 = plot(E_levelsOnN2(3,2)+5.15,I_mamp(i_Ef3_z3000,8),'d','color',cmp(2,:),'linewidth',2);
ph8446O = plot(E_levelsOnN2(4,2),I_mamp(i_Ef3_z3000,9),'ks','linewidth',2);
ph8446O2 = plot(E_levelsOnN2(4,2)+5.15,I_mamp(i_Ef3_z3000,10),'kd','linewidth',2);
...
sY = I_mamp(i_Ef3_z3000,[1 2 3 4 5 6]);
...
ylabel('Modulation amplitude (%)','fontsize',ftszLT) 
