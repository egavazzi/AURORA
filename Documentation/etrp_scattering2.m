...
v_0fcn = @(phi,theta)[cos(phi)*sin(theta),sin(phi)*sin(theta),cos(theta)];
[X,Y,Z] = sphere(181);
mu_sphere = Z;
for i1 = 2:9,
  mu_sphere(mu_sphere(:)<mu_lims(i1)) = i1;
end
colormap(jet(8))
psi = 31/180*pi;
v00 = 1.15*v_0fcn(-142*pi/180,160*pi/180);
clf
subplot(1,2,1)
surf(X,Y,Z,mu_sphere-1),shading flat
view(vaz,-vel/2)                          
hold on
v2p = v00/norm(v00);
rot_e = cross(v00/norm(v00),[0 -1 0]);
rot_e = rot_e/norm(rot_e);            
vp = (rot_around_v(rot_e,psi)*v2p')';
R_arc = r_arc(v2p,vp,[0 0 0]',1);
arrow([0 0 0],v00,'edgecolor','k')         
axis([-1 1 -1 1 -1 1]*1.1)
plot3(R_arc(:,1),R_arc(:,2),R_arc(:,3),'k')
for i1 = 1:360,
  v4psi(i1,:) =  (rot_around_v(v2p/norm(v2p),i1/180*pi)*vp');
end
plot3(v4psi(:,1),v4psi(:,2),v4psi(:,3),'k-','linewidth',2) 
caxis([0.5 8.5])
arrow([0 0 1.3],[0 0 -1.2],'edgecolor','m','width',2)

v00 = 1.15*v_0fcn(-142*pi/180,131*pi/180);
colorbar
caxis([0.5 8.5])
text(-.8,-1.05,-1,'\psi','fontsize',14)
...
colorbar
caxis([0.5 8.5])
text(-.8,-1.05,-.75,'\psi','fontsize',14)
