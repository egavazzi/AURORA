function Ie = Ie_PnL_flickeringStapered(t,E,IeEtot,Ein,dEin,w,z0,mu,mu0)
% IE_PNL_FLICKERINGSTAPERED - On-off modulated P-n-L flickering spectra
% for electrons. Ie_PnL_flickeringHtapered produces electron spectra with
% constant number-flux from the highest energy of Ein down
% low-energies with an erf-tapering of (erf( (E-Ein(1) )/40 )/2 + 1/2 )
...
% 
% Calling:
%  Ie = Ie_PnL_flickeringStapered(t,E,IeEtot,Ein,dEin,w,z0,mu,mu0)
% Input:
...
Ie = IePnL(iE)*( (1+square(w*t_shifted-pi/2))/2 .* ...
                 (t_shifted>0) * ...
