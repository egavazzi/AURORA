function Ie = Ie_PnL_flickering(t,E,IeEtot,Ein,dEin,w,z0,mu,mu0)
% IE_PNL_FLICKERING - Peticolas and Lummerzheim flickering spectra
% for electrons. Ie_PnL_flickering produces electron spectra with
...
% at w (angular frequency) with a square wave at all energies at
% a distance of z0 above, allowing for time-dispersion due to
% collisionless transport. Peticolas and Lummerzheim [2000] used
% this type of spectra to model field-aligned bursts (FAB) of
% electron precipitation for a study of the time-varying ionization
% and excitation of the ionosphere.
% 
% Calling:
%  Ie = Ie_PnL_flickering(t,E,IeEtot,Ein,dEin,w,z0,mu,mu0)
% Input:
...
Ie = IePnL(iE)*(1+square(w*t_shifted))/2.*(t_shifted>0)*double(Ein(1)<=E)*double(E<=Ein(end));