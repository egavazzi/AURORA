function Ie = Ie_PnL_flickeringH(t,E,IeEtot,Ein,dEin,w,z0,mu,mu0)
% IE_PNL_FLICKERINGH - harmonically modulated P-n-L flickering spectra
...
% at w (angular frequency) with a (1-cos(w*t/2)^2) wave at all
% energies at a distance of z0 above, allowing for time-dispersion
% due to collisionless transport. This modified Peticolas and
% Lummerzheim [2000] spectra is suitable to model low-energy-tail
...
% 
% Calling:
%  Ie = Ie_PnL_flickeringH(t,E,IeEtot,Ein,dEin,w,z0,mu,mu0)
% Input:
...
Ie = IePnL(iE)*(1-cos(w/2*t_shifted).^2).*(t_shifted>0)*double(Ein(1)<=E)*double(E<=Ein(end));
