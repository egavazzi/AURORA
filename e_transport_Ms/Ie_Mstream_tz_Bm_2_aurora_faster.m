function [Ie_ztE,mu_pars_out] = Ie_Mstream_tz_Bm_2_aurora_faster(h,mag_ze,B,E,mu_lims,mu_pars,t,I0,Ie_primary,p_e_q,ne,Te,OPS,varargin)
% Ie_Mstream_tz_2_aurora - time-dependent multi-stream electron transport
% 
% Calling: 
%  [Ie_ztEG,mu_pars_out] = Ie_Mstream_tz_2_aurora(h,mag_ze,B,E,mu_lims,mu_scatterings,t,...
...
%% 0, Options-handling and other preparatory set-ups
%  Variable initializations et al.
...
%% 1.1, Calculation of pitch-angle-stream-scattering matrices
% Here the probability-matrices (probabillities that an electron with
% pitch-angle theta scattering with scattering angle phi ends up in
% a pitch-angle-stream, and the weighting of the e-flux inside each
% stream, and the average of cos(theta)) is made
if iscell(mu_pars) && numel(size(mu_pars{1})) == 3
  %  If the appropriate input is given extract those
  Pmu2mup     = mu_pars{1};
  theta2beamW = mu_pars{2};
  mu_pars_out = mu_pars;
else
  % ...otherwise we have to calculate these. This calculation is a
  % bit time-consuming and we should not end up here, for each
  % pitch-angle-stream setup used we should calculate and store
  % that output to avoid redoing this base-work.
  % TODO: put in a warning here so that good input from good
  % working habits are encouraged.
...
%% 1.2 Calculate the time-of-flight "diffusion coefficients" for
%  the pitch-angle-streams at each energy, this is not too
%  time-consuming, seems to take ~5 s.
D_e = time_of_flight_DOMS_diffusion(E,gradE,theta_lims,0);
% Scale this diffusion (down)
D_e = D_e.*CscD_e;

%% 1.3 Calculate the pitch-angle transfer due to magnetic
%  mirroring. 
...
%   end
  ...
  for i_mu = 1:numel(c_o_mu),
    Ie_p{i_mu} = Ie_primary{i_mu}(t/2+t([2:end,end])/2,E(iE));% *gradE(iE);
  end
  ...
  [Ie_zt] = de_M_stream_CNztusD(h/cos(mag_ze),t,c_o_mu,...
                                ...
                                A,elsc_b2b,...
                                D_e(iE,:));
% $$$   [Ie_zt] = de_M_stream_CNztusDB(h/cos(mag_ze),t,c_o_mu,...
% $$$                                  Ie_p,...
% $$$                                  I0_beams,...
% $$$                                  v_of_E(E(iE)),...
% $$$                                  deg_Ie(:,:,iE)+repmat(p_e_Q(:),size(t)),...
% $$$                                  A,elsc_b2b,Mi2im1,Mi2ip1,...
% $$$                                  D_e(iE,:));
  ...
  for i1 = 1:5:length(varargin),
    ...
    for idE2 = idE:size(dE_i,1),
      ...
      if ( ~isempty(i_upper) && i_upper(1) < iE )
        ...
        for iI = i_f_ds:iE-1,
          deg_Ie(:,:,iI) = ( deg_Ie(:,:,iI) + Ionizing*deg_sp(iI) ); % Forward-scattering primary-e
          if e_ionized_distribution(iI) == 0
            break
          end
        end
