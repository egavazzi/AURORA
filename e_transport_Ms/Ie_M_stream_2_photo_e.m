function [Ie_zE,mu_pars_out] = Ie_M_stream_2_photo_e(h,mag_ze,B,E,mu_lims,mu_pars,Ie_primary,p_e_q,ne,Te,varargin)
% Ie_M_stream_2_photo_e - 2-hemispheres steady-state multi-stream electron transport
% This variant of the steady-state multi-stream electron transport
% is tailored for the case of closed magnetic field-lines with
% production of photo-electrons in the altitude-range of interest,
% but without "auroral" electron sources above (perhaps better
% phrased as: between?) the ionospheres.
% 
% Calling:  [Ie_zE,mu_pars_out] = Ie_M_stream_2_photo_e(h,mag_ze,E,mu_lims,mu_pars,Ie_primary,p_e_q,ne,Te,...
...
%  B       - magnetic-field-strength (T), double array [nZ x 1]
...
% 
% Example: photo_e_calcs_2hemispheres
%
% SEE also: e_scattering_beamdistribution, Etrp_*.m
% 

%  Copyright � Bjorn Gustavsson 20200114, bjorn.gustavsson@uit.no
...
Ie_zEpdE = zeros(numel(h)*numel(mu_lims(1:end-1)),1);
...
%   end
  ...
  % spatial gradient
  %    [Ie_ziE] = de_M_stream_CNzt(h/cos(mag_ze),t,c_o_mu,...
  %                               Ie_p,...
  %                               I0_beams,...
  %                               v_of_E(E(iE)),...
  %                               deg_Ie(:,iE)+repmat(p_e_Q(:),size(t)),...
  %                               A,elsc_b2b,...
  %                               ne.*L(:,iE),ne.*dLdE(:,iE),Ie_zEpdE,DE,...
  %                               0);
  ...
  [Ie_ziE] = de_M_stream2HS_us(h/abs(cos(mag_ze)),c_o_mu,...
                               Ie_p,...
                               deg_Ie(:,iE)+(p_e_Q(:)),...
                               A,elsc_b2b,...
                               Mi2im1,Mi2ip1, ...
                               0);
% $$$   [Ie_ziE] = de_M_stream_CNztus(h/cos(mag_ze),c_o_mu,...
% $$$                                Ie_p,...
% $$$                                deg_Ie(:,iE)+(p_e_Q(:)),...
% $$$                                A,elsc_b2b,...
% $$$                                0);
...
% $$$                                0);
  ...
  if iE > 1
    ...
    deg_Ie(:,iE-1) = deg_Ie(:,iE-1) + ... % dEds_ee(E(iE),ne,Te)/gradE(iE).*Ie_zE(:,iE).*gradient(h);
        repmat(dEds_ee(E(iE),ne,Te)/gradE(iE),size(c_o_mu(:),1),1).*Ie_zE(:,iE);
    %repmat(dEds_ee(E(iE),ne,Te)/gradE(iE).*gradient(h),size(c_o_mu(:),1),size(t,2)).*Ie_zE(:,iE);
  end 
    ...
    for idE2 = idE:size(dE_i,1),
      ...
      if ( ~isempty(i_upper) && i_upper(1) < iE )
        ...
        if second_e_isotropic % TODO: Insert proper 2nd-ary e
          ...
          Ionizing = repmat(n_i,numel(c_o_mu),1).*(xs_i(idE2,iE) * Ie_ziE);% Altitude-profiles of total ionizations
          for i_mu = numel(c_o_mu):-1:1,
...
% $$$                          repmat(n_i,numel(c_o_mu),1).*(xs_i(idE2,iE) * Ie_ziE));
        ...
        % second_e_spectra = feval(second_e_fcn,E,E(iE)+gradE(iE)/2);
        % second_e_spectra = feval(second_e_fcn,E,E(iE)+gradE(iE)/2);
        % second_e_spectra = feval(second_e_fcn,E,E(iE),dE_i(idE2,1),'s');
        ...
        % the species
% $$$         for iI = 1:iE-1,
% $$$           deg_Ie(:,iI) = deg_Ie(:,iI) + dE_i(idE2,2)*Ionization*e_ionized_distribution(iI);
% $$$           if e_ionized_distribution(iI) == 0
% $$$             break
% $$$           end
% $$$         end
% for iI = ,
        %deg_Ie(:,1:(iE-1)) = deg_Ie(:,1:(iE-1)) + dE_i(idE2,2)*Ionization*e_ionized_distribution(1:(iE-1));
%tic
%        deg_Ie(:,1:(iE-1)) = ( deg_Ie(:,1:(iE-1)) + ...
%                               Ionization*second_e_spectra(1:(iE-1)) +... % Isotropic secondaries
%                               Ionizing*deg_sp(1:(iE-1)) ); % Forward-scattering primary-e
%toc
%tic
        % 2 Smaller outer products seems to be about 30-35% faster
        ...
        %toc
        % This line eats up approximately 12 % of the running time:
        % if ~all(isfinite(deg_Ie(:)))
        %   keyboard
        % end
        %if e_ionized_distribution(iI) == 0
        %if e_ionized_distribution(iI) == 0
