function [Ie_z] = de_M_streamB_us(h_atm,mu,I_top_of_mu,QC_mu,A,B_b2b,Mi_2_im1,Mi_2_ip1,lowerBvals)
  ...
  for i3 = 1:size(B_b2b,2),
    if i2 ~= i3
      if mu(i2) < 0 % down-ward fluxes,
        ...
        if i2 == i3 + 1
          Bb2b = Bb2b + diag([0;-C_Mirror*Mi_2_ip1{i2}]./diff(h4diff));
        elseif i2 == i3 - 1
          Bb2b = Bb2b + diag([0;-C_Mirror*Mi_2_im1{i2}]./diff(h4diff));
        end
      else % up-ward fluxes
        % shift half-step down
        Bb2b = diag(-(B_b2b([1,1:end-1],i2,i3)/2 + B_b2b(:,i2,i3)/2));% diag(-B_b2b(:,i2,i3));
        if i2 == i3 + 1
          % Bb2b = Bb2b + diag([-C_Mirror*Mi_2_ip1{i2};0]./diff(h4diff));
          Bb2b = Bb2b + diag([0;-C_Mirror*Mi_2_ip1{i2}]./diff(h4diff));
        elseif i2 == i3 - 1
          % s_Bb2b = Bb2b + diag([-C_Mirror*Mi_2_im1{i2};0]./diff(h4diff));
          s_Bb2b = Bb2b + diag([0;-C_Mirror*Mi_2_im1{i2}]./diff(h4diff));
        end
      end
	...
	Ac = Au+diag([0;C_Mirror*Mi_2_ip1{i2}+C_Mirror*Mi_2_im1{i2}]./diff(h4diff));
        Bb2b = diag(-(B_b2b([1,1:end-1],i2,i3)/2 + B_b2b(:,i2,i3)/2));% diag(-B_b2b(:,i2,i3));
      end
