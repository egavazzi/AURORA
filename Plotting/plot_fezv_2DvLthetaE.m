function ph = plot_fezv_2DvLthetaE(h_atm,E,dE,feZE,BeamW,mu_lims,iZ)
...
%% 
ph = plot((E(1:size(feZE,2))),...
          max(fe_max-10,log10(fe_alt([1:end,end],:))));
set(gca,'xscale','log')
ylabel('(e^{-1}s^3/m^6)','fontsize',ftsz);
...
for itheta = (numel(mu_lims)-1):-1:1
  lstr{itheta} = sprintf('%2.1f - %2.1f',180-acos(mu_lims(itheta))*180/pi,180-acos(mu_lims(itheta+1))*180/pi);
end
legend(ph,lstr)
