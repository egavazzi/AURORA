function output = animate_IeEztE_3DEzoft(t,h_atm,E,Ie_ztE,dE,BeamSA,cx_lims,spp, theta_strs,movieOut)
...
% 
% Calling:
%  clims = animate_IeEztE_3DEzoft(t,h_atm,E,Ie_ztE,dE,BeamSA,cx_lims,spp, theta_strs)
% Input:
...
for i_t = 1:numel(t),
  ...
  for i1 = 1:size(spp,1),     
    ...
    pcolor(E,...
           h_atm/1e3,...
           log10(max(0,real(squeeze(Ie_ztE((1:numel(h_atm))+(i1-1)*numel(h_atm),i_t,:))./(ones(size(h_atm))*(dE./(E+dE/2)))/BeamSA(i1))))),
    shading flat
    if isempty(cx_lims)
      clims(i_t,i1,:) = caxis;
      caxis([-4 0]+max(caxis))
