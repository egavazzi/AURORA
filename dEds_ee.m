function Le = dEds_ee(E,ne,Te)
% dEds_ee - suprathermal electron energy loss function in e-e-collisions 
%  This function is the electron energy loss function due to
...
% Output:
%  Le - electron energy loss-function [eV/m], [nZ x nE]
% 
% The paper by Schwartz and Nesbith use electron density in cm^-3,
% here the constant is rescaled to use m^-3 instead. Further we
% calculate the loss-function, dEds(E,ne,Te) directly and not as in
% Schwartz and Nesbith dEds(E,ne,Te)/ne.
% 
% References:
...
% Copyright ,A) B. Gustavsson 20100527 - 20180528
...
if ( length(E) > 1 )
  ...
  Le = real(3.0271e-10*ne.^.97.*((E-Ee)./(E-.53*Ee)).^2.36./E.^.44./v_of_E(E));
